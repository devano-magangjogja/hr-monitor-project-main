...
@section('page-subtitle', 'Kerjakan tugas harian/custom dan unggah bukti postingan')
@section('sidebar')
...
<div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
    <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <p class="text-xs font-medium text-gray-500 mb-1">Akun Saya</p>
        <p class="text-2xl font-bold text-gray-800">{{ $stats['total_accounts'] }}</p>
        <p class="text-[11px] text-gray-400 mt-0.5">tanggung jawab saya</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        ...
        <p class="text-[11px] text-gray-400 mt-0.5">lolos PM</p>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    <div class="flex border-b border-gray-200 overflow-x-auto">
        <a href="{{ route('member.sosmed.index', ['tab' => 'tasks']) }}"
           class="flex items-center gap-2 px-5 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition
            ...
            Tugas Saya ({{ $myTasks->count() }})
        </a>
        <a href="{{ route('member.sosmed.index', ['tab' => 'accounts']) }}"
           class="flex items-center gap-2 px-5 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition
    ...
    <div class="p-5">
        <div class="mb-4">
            <h3 class="text-sm font-semibold text-gray-800">Daftar Tugas Harian & Custom</h3>
            <p class="text-xs text-gray-500 mt-0.5">Selesaikan tugas dan masukkan URL/Link hasil konten untuk diverifikasi oleh PM Anda.</p>
        </div>
            ...
            <table class="w-full text-sm min-w-[700px]">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 w-44">Bukti / Link Hasil</th>
                ...
                <tbody class="divide-y divide-gray-100">
                        ...
                        <td class="px-4 py-3.5 text-right">
                            @if(in_array($t->status, ['pending', 'rejected']))
                                <button onclick="openSubmitModal({{ $t->id }}, '{{ addslashes($t->title) }}', '{{ addslashes($t->link_upload ?? '') }}')"
                                    ...
                                    {{ $t->status === 'rejected' ? 'Kirim Ulang' : 'Selesaikan' }}
                                </button>
                            @else
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-400">Belum ada tugas sosmed yang ditugaskan kepada Anda.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <table class="w-full text-sm min-w-[600px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/80 transition">
                        ...
                        <td class="px-4 py-3.5 text-xs text-gray-700">
                            {{ $acc->pmUser?->name ?? 'Admin / HR' }}
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="4" class="px-4 py-8 text-center text-sm text-gray-400">Belum ada akun yang di-assign kepada Anda.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="modal-submit" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-base font-bold text-gray-800">Submit Bukti Pengerjaan</h3>
        ...
        <form id="form-submit" method="POST" action="" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="url" name="link_upload" id="submit-link-upload" required placeholder="https://instagram.com/p/xxx"
...
function openSubmitModal(taskId, title, currentLink) {
    ...
    document.getElementById('form-submit').action = `/member/sosmed/tasks/${taskId}/submit`;
