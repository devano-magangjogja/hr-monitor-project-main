...
{{-- Back + Header Profil Assistant --}}
<div class="mb-6">
    <a href="{{ route('staff.assistant-progress') }}?date_from={{ $dateFrom }}&date_to={{ $dateTo }}"
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-purple-50 text-purple-700">
                    HR Assistant
                </span>
            </div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
    <div class="bg-white rounded-xl border border-gray-200 px-4 py-3 flex items-center gap-3 h-full hover:shadow-md transition-shadow duration-200">
        ...
        <div class="min-w-0">
            <p class="text-xs text-gray-400 truncate">Total</p>
    ...
    <div class="space-y-4">
        @foreach($tasksByDate as $date => $dayTasks)
            @php
                ...
                $dayCompleted = $dayTasks->filter(fn($t) =>
                    ($t->assignments->first()?->is_completed ?? 'pending') === 'completed'
                )->count();
                $dayNotDone   = $dayTasks->filter(fn($t) =>
                    ($t->assignments->first()?->is_completed ?? 'pending') === 'not_done'
                )->count();
