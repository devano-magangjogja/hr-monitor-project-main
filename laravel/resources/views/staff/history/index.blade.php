            ...
            <table class="w-full text-xs sm:text-sm min-w-full">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th
                            class="text-left px-3 sm:px-6 py-2.5 sm:py-3.5 font-semibold text-gray-600 w-24 sm:w-32 whitespace-nowrap">
                            Tanggal</th>
                        <th
                            class="text-left px-3 sm:px-6 py-2.5 sm:py-3.5 font-semibold text-gray-600 w-auto min-w-[150px]">
                            Judul</th>
                        <th
                            class="text-left px-3 sm:px-6 py-2.5 sm:py-3.5 font-semibold text-gray-600 w-24 sm:w-32 whitespace-nowrap">
                            Sumber</th>
                        <th
                            class="text-left px-3 sm:px-6 py-2.5 sm:py-3.5 font-semibold text-gray-600 w-32 sm:w-64 hidden sm:table-cell">
                            Catatan</th>
                        <th
                            class="text-left px-3 sm:px-6 py-2.5 sm:py-3.5 font-semibold text-gray-600 w-28 sm:w-40 whitespace-nowrap">
                            Status</th>
                        <th
                            class="text-right px-3 sm:px-6 py-2.5 sm:py-3.5 font-semibold text-gray-600 w-12 sm:w-16 whitespace-nowrap">
                            Detail</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($tasks as $task)
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            {{-- Tanggal --}}
                            <td class="px-3 sm:px-6 py-3 sm:py-4 w-24 sm:w-32 text-gray-500 text-xs whitespace-nowrap">
                                {{ $task->task_date->translatedFormat('d M Y') }}
                            </td>

                            {{-- Judul --}}
                            <td class="px-3 sm:px-6 py-3 sm:py-4 w-auto">
                                <div class="truncate max-w-[120px] sm:max-w-xs font-medium text-gray-800 text-xs sm:text-sm"
                                    title="{{ $task->title }}">
                                    {{ $task->title }}
                                </div>
                            </td>
                            ...
                            {{-- Sumber --}}
                            <td class="px-3 sm:px-6 py-3 sm:py-4 w-24 sm:w-32 whitespace-nowrap">
                                @if($task->type === 'self')
                                    <span
                                        class="inline-flex items-center px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                                        Mandiri
                                    </span>
                                @elseif($task->type === 'assigned')
                                    <span
                                        class="inline-flex items-center px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700"
                                ...
                                @elseif($task->type === 'default')
                                    <span
                                        class="inline-flex items-center px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-xs font-medium bg-purple-50 text-purple-700">
                                        Rutin
                                    </span>
                                @endif
                            </td>
                            ...
                            {{-- Catatan --}}
                            <td class="px-3 sm:px-6 py-3 sm:py-4 w-32 sm:w-64 hidden sm:table-cell">
                                <div class="truncate max-w-[100px] sm:max-w-[200px] text-gray-500 text-xs"
                            ...
                            {{-- Status --}}
                            <td class="px-3 sm:px-6 py-3 sm:py-4 w-28 sm:w-40 whitespace-nowrap">
                                <x-task-status-badge :status="$assignment?->is_completed ?? 'pending'"
                                    :completedAt="$assignment?->completed_at" />
                            </td>

                            {{-- Detail --}}
                            <td class="px-3 sm:px-6 py-3 sm:py-4 w-12 sm:w-16 text-right">
                        ...
                        <tr>
                            <td colspan="6" class="px-3 sm:px-6 py-14 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    @if(!$date && !$search)
                                        <p class="text-xs text-gray-400">Riwayat akan muncul setelah kamu menyelesaikan tugas hari
                                            ini.</p>
                                    @endif
                                </div>
                            </td>
