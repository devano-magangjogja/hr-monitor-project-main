...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-6">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <button type="button" onclick="document.getElementById('account-modal').classList.remove('hidden')"
            class="h-10 px-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-semibold">+ Ajukan
            Akun</button>
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm table-fixed">
                <thead class="bg-gray-50 border-b border-gray-200 text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-5 py-3 text-left w-[15%]">Platform</th>
                        <th class="px-5 py-3 text-left w-[22%]">Nama Akun</th>
                        <th class="px-5 py-3 text-left w-[25%]">Email</th>
                        <th class="px-5 py-3 text-left w-[18%]">Status</th>
                        <th class="px-5 py-3 text-left w-[20%]">Catatan Penolakan</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-4">
                            <span
                                class="inline-flex px-2 py-1 rounded-full text-xs font-semibold {{ $status === 'approved' ? 'bg-emerald-50 text-emerald-700' : ($status === 'pending' ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700') }}">
                                {{ $status === 'approved' ? 'Disetujui' : ($status === 'pending' ? 'Menunggu verifikasi' : 'Ditolak') }}
                            </span>
                        </td>
                        <td class="px-5 py-4">
                            <span
                                class="text-xs truncate block {{ $status === 'rejected' ? 'text-rose-600' : 'text-gray-400' }}"
                                title="{{ $account->rejection_note ?: '-' }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-5 py-12 text-center text-sm text-gray-400">Belum ada pengajuan akun.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div id="account-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/50"
            onclick="document.getElementById('account-modal').classList.add('hidden')"></div>
        <form method="POST" action="{{ route(request()->segment(1) . '.accounts.store') }}"
            class="relative z-10 w-full max-w-lg bg-white rounded-xl shadow-xl p-6 pt-1 space-y-4">
            @csrf
            <div class="flex items-center justify-between">
                <h2 class="font-bold text-gray-800">Ajukan Akun Baru</h2><button type="button"
                    onclick="document.getElementById('account-modal').classList.add('hidden')"
                    class="text-gray-400 text-xl">&times;</button>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label class="text-xs font-semibold text-gray-700 sm:col-span-2">
                    Platform *
                    <select name="platform" id="own-platform" required
                        onchange="toggleCustomPlatform(this, 'own-custom-platform')"
                ...
                <label id="own-custom-platform" class="hidden text-xs font-semibold text-gray-700 sm:col-span-2">
                    Nama Platform Custom *
                    <input name="custom_platform" maxlength="50"
                        class="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                </label>

                {{-- Nama Akun diisi manual --}}
                <label class="text-xs font-semibold text-gray-700 sm:col-span-2">
                    Nama Akun *
                    <input name="name" type="text" required maxlength="200" placeholder="Contoh: @republikweb_net"
                        class="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                </label>
                ...
                <label class="text-xs font-semibold text-gray-700 sm:col-span-2">
                    ...
                    <span class="relative block mt-1">
                        <input id="own-password" name="password" type="password" required
                            class="w-full border border-gray-200 rounded-lg pl-3 pr-10 py-2 text-sm">
                        <button type="button" onclick="togglePassword('own-password', this)"
                            class="absolute inset-y-0 right-0 px-3 text-gray-400" title="Lihat password">
            ...
            <button
                class="w-full py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-sm font-semibold">Kirim
                Pengajuan</button>
        </form>
    </div>
    <script>
