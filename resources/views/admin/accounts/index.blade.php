...
═══════════════════════════════════════════════════════════════ --}}
    @if ($tab === 'pending')
        <div class="bg-white rounded-xl border border-amber-200 shadow-sm overflow-hidden">
            ...
            <div class="px-4 sm:px-6 py-4 border-b border-amber-100 bg-amber-50/50">
                ...
                <p class="text-xs text-gray-500 mt-0.5">Lengkapi atribut keamanan sebelum menyetujui atau menolak pengajuan.
                </p>
            </div>
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <div class="text-right">
                                ...
                                <span
                                    class="text-[10px] text-gray-400 block">{{ $pending->creator?->role_label ?? '' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-2.5 space-y-1.5 text-xs">
                            <div class="flex justify-between items-center">
                                ...
                                <span
                                    class="font-medium text-gray-700 truncate max-w-[200px]">{{ $pending->email ?: '-' }}</span>
                            </div>
                            <div class="flex justify-between items-center" x-data="{ show: false }">
                                ...
                                @if ($pending->password)
                                    <div class="flex items-center gap-1.5">
                                        <span x-show="!show"
                                            class="font-mono text-gray-400 tracking-widest">••••••••</span>
                            ...
                            <tr class="align-top hover:bg-amber-50/20 transition">
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    <div class="text-xs text-gray-500 mt-1 flex items-center gap-1">
                                        <span
                                            class="font-medium text-gray-700">{{ $pending->creator?->name ?? '-' }}</span>
                                    </div>
                                    <div class="text-[10px] text-gray-400">{{ $pending->creator?->role_label ?? '' }}
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-4" x-data="{ show: false }">
                                    @if ($pending->password)
                                        <div class="flex items-center gap-2">
                                            <span x-show="!show"
                                                class="font-mono text-xs tracking-widest text-gray-400">••••••••</span>
                                            <span x-show="show" x-cloak
        ...
        <div id="modal-approve-account" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative z-10 w-full max-w-md rounded-xl bg-white shadow-2xl p-6">
                ...
                <p class="mt-1 text-xs text-gray-500">Approve akun <strong id="approve-account-name"></strong> setelah
                    data
                    keamanan diisi.</p>
        ...
        <div id="modal-reject-account" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative z-10 w-full max-w-md rounded-xl bg-white shadow-2xl p-6">
                ...
                <p class="mt-1 text-xs text-gray-500">Berikan catatan untuk pengaju agar penolakan dapat ditindaklanjuti.
                </p>
                <form id="form-reject-account" method="POST" class="mt-5 space-y-4">
                    @csrf @method('PATCH')
                    <input type="hidden" name="verification_status" value="rejected">
                    <textarea name="rejection_note" required maxlength="1000" rows="4" placeholder="Tuliskan alasan penolakan..."
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></textarea>
                    <div class="flex gap-2">
                        <button type="button" onclick="closeAccountDecisionModals()"
                            class="flex-1 rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700">Batal</button>
                        <button
                            class="flex-1 rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white">Reject</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        ═══════════════════════════════════════════════════════════ --}}
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="px-4 sm:px-6 py-4 border-b border-gray-100 bg-indigo-50/40">
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Klik sebuah brand untuk melihat akun-akun di dalamnya. Beberapa
                        brand
                            ...
                            <div class="flex items-center gap-1 px-4 sm:px-6 hover:bg-gray-50 transition">
                                <button type="button" onclick="toggleBrandDetail({{ $i }})"
                                    ...
                                    <span class="flex items-center gap-2 min-w-0">
                                        @if ($br->logo_path)
                                            <img src="{{ asset('storage/' . $br->logo_path) }}"
                                                alt="Logo {{ $br->name }}"
                                ...
                                <button type="button"
                                    onclick="openEditBrandModal({{ json_encode([
                                        'id' => $br->id,
                                        'name' => $br->name,
                    ...
                    @empty
                        <div class="px-6 py-12 text-center text-gray-400">
                            @if ($search ?? null)
                                ...
                                <p class="text-xs text-gray-400 mt-1">Tidak ada brand yang cocok dengan
                                    "{{ $search }}".</p>
                            @else
                                <p class="font-medium text-gray-600">Belum ada brand</p>
                                <p class="text-xs text-gray-400 mt-1">Klik tombol "Tambah Brand" di atas untuk menambahkan
                                    brand.</p>
                            @endif
                        </div>
                    @endforelse
    ...
    ═══════════════════════════════════════════════════════════ --}}
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div
                    class="px-4 sm:px-6 py-2.5 border-b border-gray-100 flex flex-wrap items-center gap-4 text-[11px] text-gray-500">
                    ...
                    <span class="inline-flex items-center gap-1.5"><span
                        ...
                        Belum Dikelola</span>
                    <span class="inline-flex items-center gap-1.5"><span
                            class="w-2.5 h-2.5 rounded-full bg-red-500"></span> Sudah
                        Dikelola</span>
                </div>
            ...
            <form method="POST" action="{{ route($accountPrefix . '.accounts.store') }}"
                ...
                <div>
                    ...
                    <input type="text" name="link" placeholder="https://instagram.com/republikweb_net"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:outline-none">
                    <p class="text-[11px] text-gray-400 mt-1">Link ini dapat langsung diklik pada tabel untuk membuka
                        profil
                        akun.</p>
                </div>
                ...
                <label class="inline-flex items-center gap-2 text-xs font-semibold text-gray-700">
                    <input type="checkbox" name="two_factor_enabled" value="1" class="rounded border-gray-300"> 2FA
                    aktif
                </label>
        ...
        <div
            ...
            <form id="form-edit-account" method="POST" action="" class="p-6 pt-1 space-y-4 overflow-y-auto">
                ...
                <div>
                    ...
                    <input type="text" name="custom_platform" id="edit-custom-platform"
                        placeholder="Masukkan nama platform"
                        class="hidden mt-2 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:outline-none">
                </div>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1.5">Link Akun (URL Profil)</label>
                    <input type="text" name="link" id="edit-acc-link"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="password" id="edit-acc-password"
                            placeholder="Kosongkan jika tetap sama"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-primary-500 focus:outline-none">
                    </div>
                </div>
    ...
    <div id="modal-edit-brand" class="hidden fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10 overflow-hidden">
            ...
            <form id="form-edit-brand" method="POST" action="" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <img id="edit-brand-logo-preview" alt="Logo brand"
                            class="hidden w-14 h-14 shrink-0 rounded-lg border border-gray-200 bg-white p-1 object-contain">
                        <input type="file" id="edit-brand-logo-input" name="logo"
                            accept="image/png,image/jpeg,image/webp" onchange="previewEditBrandLogo(this)"
                            class="block w-full text-xs text-gray-600 cursor-pointer file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-50 file:px-3 file:py-2 file:text-xs file:font-semibold file:text-indigo-700 hover:file:bg-indigo-100 file:cursor-pointer">
                    </div>
    ...
    <script>
        ...
        function openEditAccountModal(data) {
            ...
            window.dispatchEvent(new CustomEvent('set-edit-brand', {
                detail: data.brand || ''
            }));
            const standardPlatforms = Array.from(document.getElementById('edit-acc-platform').options).map(option => option
                .value);
            ...
            toggleAdminCustomPlatform(document.getElementById('edit-acc-platform'), 'edit-custom-platform',
                isCustomPlatform ? data.platform : '');
    ...
    @if ($tab === 'rejected')
        <div class="bg-white rounded-xl border border-rose-200 shadow-sm overflow-hidden">
            <div class="px-4 sm:px-6 py-4 border-b border-rose-100 bg-rose-50/50">
                ...
                <p class="text-xs text-gray-500 mt-0.5">Akun-akun ini telah ditolak dan tidak muncul di daftar manajemen
                    maupun
                    kelola sosmed.</p>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-xs sm:text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-10 text-center text-sm text-gray-400">Tidak ada
                                    pengajuan yang
                                    ditolak.</td>
                            </tr>
                        @endforelse
