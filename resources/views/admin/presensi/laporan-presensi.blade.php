...
@section('content')
    ...
    <div class="print:hidden bg-white rounded-xl border border-gray-200 p-4 mb-6 shadow-sm">
        <form method="GET" action="{{ route('admin.presensi.laporan') }}"
                    ...
                    <tbody class="divide-y divide-gray-100 print:divide-gray-200">
                        @forelse($rekapPemagang as $item)
                            @php
                                ...
                                // Fallback jika field belum tersedia dari controller
                                $terlambatPagi = $item->terlambat_pagi ?? ($item->terlambat ?? 0);
    ...
    @if (request('tab') === 'riwayat')
        <div
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm print:text-xs min-w-[700px] print:min-w-full">
                    <thead>
                        <tr
                            class="bg-gray-50 print:bg-gray-100 border-b border-gray-200 print:border-gray-300 text-xs print:text-[11px] font-semibold text-gray-600 print:text-gray-800 uppercase tracking-wider">
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Pemagang</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Divisi</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Lokasi</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Shift</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Tanggal</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Waktu Masuk</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Status</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Pencatat</th>
                            <th class="px-3 sm:px-6 py-3 print:px-3 print:py-2">Catatan</th>
                            <th class="px-3 sm:px-6 py-3 print:hidden text-right">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 print:divide-gray-200">
                        @forelse($logs as $log)
                            ...
                            @endphp
                            <tr class="hover:bg-gray-50/80 transition">
                                <td class="px-3 sm:px-6 py-3 print:px-3 print:py-1.5">
                                ...
                                <td
                                    class="px-3 sm:px-6 py-3 print:px-3 print:py-1.5 text-xs text-gray-700 font-medium whitespace-nowrap">
                                    {{ \Carbon\Carbon::parse($log->tanggal)->locale('id')->translatedFormat('d M Y') }}
                                </td>
                                <td class="px-3 sm:px-6 py-3 print:px-3 print:py-1.5 text-xs text-gray-700 font-medium">
                                ...
                                <td class="px-3 sm:px-6 py-3 print:px-3 print:py-1.5 text-xs text-gray-600 font-medium">
                                    {{ $log->creator?->name ?? 'Staff/Admin' }}
                                </td>
                                <td
                                    class="px-3 sm:px-6 py-3 print:px-3 print:py-1.5 text-xs text-gray-500 max-w-[160px] sm:max-w-[200px] truncate">
                                    {{ $log->notes ?: '-' }}
                                </td>
                                <td class="px-3 sm:px-6 py-3 print:hidden text-right">
                                    <form method="POST" action="{{ route('admin.presensi.destroy', $log) }}"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="p-2 sm:p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"
    ...
    <div class="hidden print:block mt-8 pt-6 border-t border-gray-300 print-avoid-break">
        <div class="flex justify-between items-start text-xs text-gray-700">
            ...
            <div class="text-center w-60">
                ...
                <p class="text-[10px] text-gray-500">Penanggung Jawab / HR Manager</p>
            </div>
        </div>
    </div>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-lg max-w-md w-full mx-4">
            ...
            <form method="POST" action="{{ route('admin.presensi.bulk-delete') }}" class="p-6 space-y-4">
                ...
                <input type="hidden" name="tab" value="riwayat">
                <input type="hidden" name="delete_date" id="modalTanggal"
    ...
    {{-- ── MODAL DETAIL PRESENSI (Rekapitulasi) ──────────────────── --}}
    <div id="presensiDetailModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="p-6 space-y-5">
                ...
                {{-- Detail Keterlambatan (inti permintaan) --}}
    ...
    @push('scripts')
        <script>
            // Existing delete modal functions
            ...
            function openPresensiDetailModal(btn) {
                ...
                const telatIstirahat = parseInt(btn.dataset.telatIstirahat || 0);
                ...
                if (telatIstirahat > 0) {
                    elIst.textContent = telatIstirahat + ' kali';
                    elIst.className =
                        'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-800';
                } else {
                ...
                rateEl.className = 'font-bold ' + (rate >= 80 ? 'text-green-600' : (rate >= 60 ? 'text-amber-600' :
                    'text-red-600'));
