        ...
        <table class="w-full table-fixed text-xs sm:text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    {{-- Tanggal --}}
                    <th class="w-[100px] sm:w-[120px] px-3 sm:px-5 py-3 text-left font-semibold text-gray-600 whitespace-nowrap">
                        Tanggal
                    </th>

                    {{-- Judul --}}
                    <th class="w-[150px] sm:w-[220px] px-3 sm:px-5 py-3 text-left font-semibold text-gray-600">
                        Judul
                    </th>

                    {{-- Penerima --}}
                    <th class="w-[140px] sm:w-[180px] px-3 sm:px-5 py-3 text-left font-semibold text-gray-600">
                        Penerima
                    </th>

                    {{-- Sumber --}}
                    <th class="w-[110px] sm:w-[140px] px-3 sm:px-5 py-3 text-left font-semibold text-gray-600">
                        Sumber
                    </th>

                    {{-- Catatan --}}
                    <th class="hidden sm:table-cell px-3 sm:px-5 py-3 text-left font-semibold text-gray-600">
                        Catatan
                    </th>

                    {{-- Status --}}
                    <th class="w-[120px] sm:w-[140px] px-3 sm:px-5 py-3 text-center font-semibold text-gray-600 whitespace-nowrap">
                        Status
                    </th>

                    {{-- Aksi --}}
                    <th class="w-[95px] sm:w-[120px] px-3 sm:px-5 py-3 text-center font-semibold text-gray-600 whitespace-nowrap">
                        Aksi
                    </th>
                </tr>
            </thead>
                ...
                @forelse($tasks as $task)
                    @foreach($task->assignedUsers as $assignee)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-3 sm:px-5 py-3 sm:py-4 align-middle">
                                <div class="font-medium text-gray-700 text-xs sm:text-sm truncate" title="{{ $assignee->name }}">
                                    {{ $assignee->name }}
                                </div>
                            ...
                            <td class="px-3 sm:px-5 py-3 sm:py-4 align-middle">
                                @if($task->type === 'self')
                                    <span class="inline-flex items-center justify-center px-2 sm:px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-gray-100 text-gray-600 whitespace-nowrap">
                                        Mandiri
                                    </span>
                                @elseif($task->type === 'assigned')
                                    <span class="inline-flex items-center max-w-full px-2 sm:px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-blue-50 text-blue-700 truncate" title="{{ $task->creator?->name ?? 'Admin' }}">
                                        {{ $task->creator?->name ?? 'Admin' }}
                                    </span>
                                @elseif($task->type === 'default')
                                    <span class="inline-flex items-center justify-center px-2 sm:px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-purple-50 text-purple-700 whitespace-nowrap">
                                        Rutin
                                    </span>
                                @endif
                            </td>
                            <td class="hidden sm:table-cell px-3 sm:px-5 py-3 sm:py-4 align-middle">
                                <div class="truncate max-w-[180px] text-xs text-gray-500" title="{{ $assignment?->note ?? '-' }}">
                                    {{ $assignment?->note ?? '-' }}
                                </div>
                            </td>
