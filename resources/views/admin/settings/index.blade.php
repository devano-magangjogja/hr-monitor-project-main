...
@section('page-subtitle', 'Kelola informasi aplikasi dan link grup WhatsApp')
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <form action="{{ route('admin.settings.app-info') }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="app_name"
                       value="{{ $settings['app_name'] ?? 'HR-DWMS' }}"
            ...
            <button type="submit"
                    class="w-full py-2.5 bg-primary-600 hover:bg-primary-700 text-white
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <form action="{{ route('admin.settings.wa-groups.store') }}" method="POST"
            ...
            <div>
                <input type="text" name="label" placeholder="Nama Grup (contoh: Grup HR Staff)"
                       required maxlength="80" value="{{ old('label') }}"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                              focus:outline-none focus:ring-2 focus:ring-primary-500">
            </div>
            <div class="flex items-center gap-3">
                <input type="url" name="url" placeholder="https://chat.whatsapp.com/..."
                       required maxlength="255" value="{{ old('url') }}"
            ...
            @forelse($waGroups as $group)
                <div class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition"
                    ...
                    <form x-show="editing" x-cloak
                          action="{{ route('admin.settings.wa-groups.update', $group) }}"
                          method="POST" class="flex-1 space-y-2">
                        @csrf
                        @method('PATCH')
                        <input type="text" name="label" value="{{ $group->label }}"
                               required maxlength="80"
                               class="w-full px-2 py-1.5 border border-gray-300 rounded-lg text-xs
                                      focus:outline-none focus:ring-2 focus:ring-primary-500">
                        <input type="url" name="url" value="{{ $group->url }}"
                    ...
                    <div class="flex items-center gap-1 flex-shrink-0" x-show="!editing">
                        <button @click="editing = true"
                                class="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50
                        ...
                        <form action="{{ route('admin.settings.wa-groups.destroy', $group) }}"
                              method="POST"
                            ...
                            <button type="submit"
                                    class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50
                                           rounded-lg transition" title="Hapus">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                          d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                </svg>
