...
@section('content')
    ...
    <div class="print:hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <p class="text-sm text-gray-500">
                ...
                <span
                    class="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200">
    ...
    <div class="print:hidden bg-white rounded-xl border border-gray-200 p-4 mb-6 shadow-sm">
        <form method="GET" action="{{ route('assistant.presensi.laporan') }}"
            ...
            <div class="flex items-center gap-2">
                ...
                @if (request()->hasAny(['search', 'divisi']))
                    <a href="{{ route('assistant.presensi.laporan', ['tab' => request('tab', 'rekapitulasi')]) }}"
    ...
    <div class="print:hidden bg-white rounded-t-xl border border-b-0 border-gray-200 overflow-hidden mb-0 shadow-sm">
        <div class="flex border-b border-gray-200 overflow-x-auto scrollbar-none">
            <a href="{{ route('assistant.presensi.laporan', array_merge(request()->except('tab', 'page_rekap', 'page_logs'), ['tab' => 'rekapitulasi'])) }}"
            ...
            <a href="{{ route('assistant.presensi.laporan', array_merge(request()->except('tab', 'page_rekap', 'page_logs'), ['tab' => 'presensi_masuk'])) }}"
                class="flex items-center gap-2 px-5 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition
                ...
                <span
                    class="ml-1 px-2 py-0.5 rounded-full text-xs font-bold
                         {{ request('tab') === 'presensi_masuk' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600' }}">
                    {{ $totalPresensiMasuk }}
                </span>
            </a>
            <a href="{{ route('assistant.presensi.laporan', array_merge(request()->except('tab', 'page_rekap', 'page_logs'), ['tab' => 'presensi_istirahat'])) }}"
                class="flex items-center gap-2 px-5 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition
                ...
                <span
                    class="ml-1 px-2 py-0.5 rounded-full text-xs font-bold
                         {{ request('tab') === 'presensi_istirahat' ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 text-gray-600' }}">
                    {{ $totalPresensiIstirahat }}
                </span>
            </a>
            <a href="{{ route('assistant.presensi.laporan', array_merge(request()->except('tab', 'page_rekap', 'page_logs'), ['tab' => 'riwayat'])) }}"
                class="flex items-center gap-2 px-5 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition
            ...
            <div
                ...
                <div>
                    ...
                    <p class="text-xs print:text-[11px] text-gray-500">Ringkasan performa dan tingkat kedisiplinan
                        kehadiran setiap individu di {{ $selectedKantor }}</p>
                </div>
                <span
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-6 py-8 text-center text-gray-400 text-xs">
                                    Tidak ada data presensi pemagang di kantor ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-6 text-center text-gray-400 text-xs">
                                    Belum ada riwayat log presensi di kantor ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div id="tabel-presensi-masuk"
            ...
            <div
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">Data pemagang yang melakukan presensi masuk di {{ $selectedKantor }}
                        pada tanggal yang dipilih</p>
                </div>
            ...
            <div class="px-5 sm:px-6 py-3 border-b border-gray-100 bg-white">
                <form method="GET" action="{{ route('assistant.presensi.laporan') }}"
                    ...
                    @if (request('search'))
                        <a href="{{ route('assistant.presensi.laporan', ['tab' => 'presensi_masuk', 'kantor' => $selectedKantor]) }}"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-10 text-center">
                                    <div class="flex flex-col items-center gap-2 text-gray-400">
                                        ...
                                        <p class="text-xs">Belum ada data presensi masuk untuk tanggal ini di
                                            {{ $selectedKantor }}.</p>
                                    </div>
                                </td>
                            </tr>
        ...
        <div id="tabel-presensi-istirahat"
            ...
            <div
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">Data pemagang yang telah kembali dari istirahat di
                        {{ $selectedKantor }} pada tanggal yang dipilih</p>
                </div>
            ...
            <div class="px-5 sm:px-6 py-3 border-b border-gray-100 bg-white">
                <form method="GET" action="{{ route('assistant.presensi.laporan') }}"
                    ...
                    @if (request('search'))
                        <a href="{{ route('assistant.presensi.laporan', ['tab' => 'presensi_istirahat', 'kantor' => $selectedKantor]) }}"
                            class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"
                            title="Reset">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <table class="w-full text-left text-sm min-w-[500px]">
                    <thead>
                        <tr
                            ...
                            <th class="px-6 py-3.5">Nama Pemagang</th>
                            <th class="px-6 py-3.5">Divisi</th>
                            <th class="px-6 py-3.5">Waktu Kembali</th>
                            <th class="px-6 py-3.5">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-10 text-center">
                                    <div class="flex flex-col items-center gap-2 text-gray-400">
                                        ...
                                        <p class="text-xs">Belum ada data presensi istirahat untuk tanggal ini di
                                            {{ $selectedKantor }}.</p>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="flex justify-between items-start text-xs text-gray-700">
            ...
            <div class="text-center w-60">
                <p class="text-gray-500 mb-1">Yogyakarta,
                    {{ \Carbon\Carbon::parse($tanggal)->locale('id')->translatedFormat('d F Y') }}</p>
