...
@section('content')
    ...
    <div class="hidden print:block mb-6 border-b border-gray-300 pb-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-600">Dicetak pada:
                    {{ \Carbon\Carbon::now()->locale('id')->translatedFormat('l, d F Y - H:i') }} WIB</p>
            </div>
    ...
    <div class="print:hidden bg-white rounded-xl border border-gray-200 p-4 mb-6 shadow-sm">
        <form method="GET" action="{{ route('assistant.presensi.laporan') }}"
            ...
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama pemagang / no hp..."
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request()->hasAny(['search', 'divisi', 'shift', 'keterangan']))
                    <a href="{{ route('assistant.presensi.laporan') }}"
                        class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition" title="Reset Filter">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </a>
    ...
    <div id="tabel-rekap-pemagang"
        class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm mb-8 scroll-mt-6">
                    ...
                    @forelse($rekapPemagang as $item)
                        ...
                        <tr class="hover:bg-gray-50/80 transition">
                            ...
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700">
                                    {{ $item->datang_awal }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-green-50 text-green-700">
                                    {{ $item->tepat_waktu }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-red-50 text-red-700">
                                    {{ $item->tidak_hadir }}
                                </span>
                            </td>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm min-w-[700px]">
                <thead>
                    <tr
                        class="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                        <th class="px-6 py-3 font-semibold text-gray-600">Nama Pemagang</th>
                        <th class="px-6 py-3 font-semibold text-gray-600">Divisi</th>
                        <th class="px-6 py-3 font-semibold text-gray-600">Kantor</th>
                        <th class="px-6 py-3 font-semibold text-gray-600">Shift</th>
                        <th class="px-6 py-3 font-semibold text-gray-600">Waktu Masuk</th>
                        <th class="px-6 py-3 font-semibold text-gray-600">Status</th>
                        <th class="px-6 py-3 font-semibold text-gray-600">Catatan</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($logs as $log)
                        ...
                        <tr class="hover:bg-gray-50/80 transition">
                            <td class="px-6 py-3.5">
                                <p class="font-medium text-gray-800 text-xs sm:text-sm">
                                    {{ $pemagang ? $pemagang->nama_lengkap : 'Pemagang Dihapus' }}</p>
                                <p class="text-[11px] text-gray-400">{{ $pemagang ? $pemagang->kampus : '-' }}</p>
                            </td>
                            <td class="px-6 py-3.5">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-gray-100 text-gray-700">
                                    {{ $pemagang ? $pemagang->divisi : '-' }}
                                </span>
                            </td>
                            <td class="px-6 py-3.5">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-blue-50 text-blue-700">
                                    {{ $log->kantor ?? '-' }}
                                </span>
                            </td>
                            <td class="px-6 py-3.5">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium {{ $shiftStyle }}">
                                    {{ $log->shift }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-3.5">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border {{ $badgeStyle }}">
                                    {{ $log->keterangan }}
                                </span>
                            </td>
