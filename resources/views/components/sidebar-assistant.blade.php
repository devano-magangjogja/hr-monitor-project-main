...
<a href="{{ route('assistant.sosmed.index') }}"
    ...
    <span class="flex-1 text-left">Sosmed</span>
    @if($sosmedPending > 0)
...
{{-- Divider --}}
<div class="pt-4 pb-2">
    <p class="px-4 text-[11px] font-semibold text-slate-500 uppercase tracking-widest">AKUN SOSMED</p>
</div>

{{-- Tambah Akun Sosmed --}}
<a href="{{ route('assistant.accounts.index') }}"
    class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all
          {{ request()->routeIs('assistant.accounts.index') ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}">
    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<a href="{{ route('assistant.accounts.submissions') }}"
    class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all
          {{ request()->routeIs('assistant.accounts.submissions') ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}">
...
@php($presensiActive = request()->routeIs('assistant.presensi.*'))
