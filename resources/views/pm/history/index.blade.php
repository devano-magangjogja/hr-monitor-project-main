...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 mb-6 shadow-sm">
        <form method="GET" action="{{ route('pm.tasks.history') }}"
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full min-w-[680px] table-fixed text-xs sm:text-sm">
                {{-- HEADER --}}
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th
                            class="w-[15%] px-3 sm:px-5 py-3 sm:py-3.5 text-left font-semibold text-gray-600 whitespace-nowrap">
                            Tanggal
                        </th>
                        <th class="w-[30%] px-3 sm:px-5 py-3 sm:py-3.5 text-left font-semibold text-gray-600">
                            Judul
                        </th>
                        <th
                            class="w-[15%] px-3 sm:px-5 py-3 sm:py-3.5 text-left font-semibold text-gray-600 whitespace-nowrap">
                        ...
                        <th
                            class="hidden sm:table-cell w-[18%] px-3 sm:px-5 py-3 sm:py-3.5 text-left font-semibold text-gray-600">
                            Catatan
                        </th>
                        <th
                            class="w-[14%] px-3 sm:px-5 py-3 sm:py-3.5 text-center font-semibold text-gray-600 whitespace-nowrap">
                            Status
                        </th>
                        <th
                            class="w-[8%] px-3 sm:px-5 py-3 sm:py-3.5 text-center font-semibold text-gray-600 whitespace-nowrap">
                            Detail
                        </th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($tasks as $task)
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        {{-- Tanggal --}}
                                        <td class="px-3 sm:px-5 py-3.5 sm:py-4 text-gray-500 text-xs whitespace-nowrap align-middle">
                                            {{ $task->task_date->translatedFormat('d M Y') }}
                                        </td>

                                        {{-- Judul --}}
                                        <td class="px-3 sm:px-5 py-3.5 sm:py-4 align-middle">
                                            <div class="truncate font-medium text-gray-800 text-xs sm:text-sm" title="{{ $task->title }}">
                                                {{ $task->title }}
                                            </div>
                                        </td>

                                        {{-- Sumber --}}
                                        <td class="px-3 sm:px-5 py-3.5 sm:py-4 align-middle whitespace-nowrap">
                                            @if($task->type === 'self')
                                                <span
                                                    class="inline-flex items-center px-2 sm:px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-gray-100 text-gray-600 whitespace-nowrap">
                                                    Mandiri
                                                </span>
                                            @elseif($task->type === 'assigned')
                                                <span
                                                    ...
                                                    <span class="hidden sm:inline truncate">{{ $task->creator?->name ?? 'Admin' }}</span>
                                                    <span class="sm:hidden">Tugas</span>
                                                </span>
                                            @elseif($task->type === 'default')
                                                <span
                                                    class="inline-flex items-center px-2 sm:px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-purple-50 text-purple-700 whitespace-nowrap">
                                                    Rutin
                                                </span>
                                            @endif
                                        </td>
                                        ...
                                        {{-- Catatan --}}
                                        <td class="hidden sm:table-cell px-3 sm:px-5 py-3.5 sm:py-4 align-middle">
                                            <div class="truncate max-w-full text-gray-500 text-xs" title="{{ $assignment?->note ?? '-' }}">
                                                {{ $assignment?->note ?? '-' }}
                                            </div>
                                        </td>

                                        {{-- Status --}}
                                        <td class="px-3 sm:px-5 py-3.5 sm:py-4 align-middle text-center whitespace-nowrap">
                                        ...
                                        {{-- Detail --}}
                                        <td class="px-3 sm:px-5 py-3.5 sm:py-4 align-middle text-center">
                                            <button onclick="openDetailModal(JSON.parse(this.dataset.detail))" data-detail="{{ json_encode([
                            'title' => $task->title,
                            'kantor' => $task->kantor,
                            ...
                            'assignees' => [],
                                                ...
                                                class="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition inline-flex items-center justify-center"
                                                title="Lihat Detail">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    @if(!$date && !$search)
                                        <p class="text-xs text-gray-400">
                                            Riwayat akan muncul setelah kamu menyelesaikan tugas hari ini.
                                        </p>
                                    @endif
                                </div>
                            </td>
