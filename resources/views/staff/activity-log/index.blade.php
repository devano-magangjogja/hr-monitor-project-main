...
@section('page-subtitle', 'Pantau riwayat aktivitas untuk modul Manajemen Akun dan Sosmed')
...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm mb-4 p-3">
        <form method="GET" action="{{ route('staff.activity-log.index') }}" class="space-y-2.5">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2">
                ...
                <div>
                    <select name="module" onchange="this.form.submit()"
                        class="w-full px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="">Semua Modul (Kelola Akun & Sosmed)</option>
                ...
                <div>
                    <select name="action" onchange="this.form.submit()"
                        ...
                        <option value="unassigned" {{ request('action') == 'unassigned' ? 'selected' : '' }}>Dilepas</option>
                        <option value="submitted" {{ request('action') == 'submitted' ? 'selected' : '' }}>Diserahkan (Laporan)</option>
                        <option value="verified" {{ request('action') == 'verified' ? 'selected' : '' }}>Diverifikasi</option>
                        <option value="completed" {{ request('action') == 'completed' ? 'selected' : '' }}>Diselesaikan</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-2 pt-1 border-t border-gray-100">
                ...
                <div class="flex items-center gap-2 w-full sm:w-auto shrink-0 justify-end">
                    @if(request()->hasAny(['search', 'user_id', 'role', 'module', 'action', 'date_from', 'date_to']))
            ...
            <table class="w-full text-xs text-left">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($logs as $log)
                        @php
                            $moduleColors = [
                                'Manajemen Akun' => 'bg-indigo-50 text-indigo-700 border-indigo-200',
                        ...
                        <tr class="hover:bg-gray-50/70 transition">
                            ...
                            <td class="px-3 py-3 text-center whitespace-nowrap">
                                <form action="{{ route('staff.activity-log.destroy', $log->id) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-400">Belum ada log aktivitas yang sesuai filter.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            @forelse($logs as $log)
                ...
                <div class="p-3.5 space-y-2">
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            <span class="font-semibold text-gray-800 text-xs">{{ $log->user_name }}</span>
                            <span class="ml-1 inline-block px-1.5 py-0.2 rounded text-[9px] font-medium bg-slate-100 text-slate-600 uppercase">{{ $log->user_role }}</span>
                        </div>
                        <span class="text-[10px] text-gray-400 whitespace-nowrap">{{ $log->created_at->diffForHumans() }}</span>
                    </div>

                    <div class="flex items-center gap-1.5 flex-wrap">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border {{ $moduleStyle }}">
                            {{ $log->module }}
                        </span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $actionStyle }}">
                            {{ ucfirst($actionKey) }}
                        </span>
                        @if($log->ip_address)
                            <span class="text-[10px] text-gray-400 font-mono">({{ $log->ip_address }})</span>
                        @endif
                    </div>

                    <p class="text-xs text-gray-700 leading-relaxed">{{ $log->description }}</p>

                    <div class="flex items-center justify-between pt-1 border-t border-gray-50 text-[10px] text-gray-400">
                        <span>{{ $log->created_at->locale('id')->translatedFormat('d M Y, H:i:s') }}</span>
                        <form action="{{ route('staff.activity-log.destroy', $log->id) }}" method="POST"
                            onsubmit="return confirm('Hapus riwayat aktivitas ini?')" class="inline-block">
                            @csrf
                            @method('DELETE')
